        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h1 class="page-header">Edit Kriteria</h1>
              ...
              <section class="row">
                ...
                <?php
                  $kode_kriteria = $_GET['id_kriteria'];
                  $query = $mysqli->query("SELECT * FROM kriteria where id_kriteria='$kode_kriteria'");

                  while ($qu = mysqli_fetch_array($query)){
                    ?>
                  <!-- form sdasddtart -->
                  <form role="form" action="proseseditkriteria.php" method="post">
                  <div class = "box-body">
                  <div class ="form-group">
                    <label for="exampleInputEmail1">No</label>
                    <input type="text" disable value="<?php echo $qu['id_kriteria'] ?>" name="id_krite" class="form-control" placeholder="" disabled>
                    <input type="hidden" value="<?= $qu['id_kriteria']; ?>" name="id_kriteria">
                    </div>
                    <div class ="form-group">
                    <label for="exampleInputPassword1">Kriteria </label>
                    <input type="text"  value="<?php echo$qu['kriteria_tbl'] ?>" name="kriteria_tbl"
                    class="form-control" placeholder="" required>
                    </div>
                    <div class ="form-group">
                    <label for="exampleInputPassword1">Nilai </label>
                    <input type="text"  value="<?php echo$qu['nilai'] ?>" name="nilai"
                    class="form-control" placeholder="" required>
                    </div>
            ...
            <a href="satuan.php" class="btn btn-danger">Back</a>
            </div>
            </form>
            </section><!-- /.content -->
